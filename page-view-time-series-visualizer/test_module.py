...
def test_bar_plot_number_of_bars(self):
    rectangles = [rect for rect in self.ax.get_children() if isinstance(rect, mpl.patches.Rectangle)]
    for rect in rectangles:
        print(f"Rectangle: {rect}, Width: {rect.get_width()}, Height: {rect.get_height()}, Visible: {rect.get_visible()}")
    bar_rectangles = [rect for rect in rectangles if rect.get_width() > 0 and rect.get_height() > 0]
    actual = len(bar_rectangles)
    expected = 49
